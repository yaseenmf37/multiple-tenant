...
export default async function BlogArticlePage({
  ...
  const sections = t("blogArticle.sections", { returnObjects: true }) as { h: string; p: string }[];
  const checkoutList = t("blogArticle.checkoutList", { returnObjects: true }) as string[];
  const shareTags = t("blogArticle.shareTags", { returnObjects: true }) as string[];
  const related = t("blogArticle.related", { returnObjects: true }) as { tag: string; title: string }[];
      ...
      <main>
        ...
        <article>
          <header className="container text-center" style={{ maxWidth: "760px", paddingTop: "clamp(36px,4vw,60px)" }}>
            <LocalizedLink href={ROUTES.BLOG} className="flex items-center justify-center gap-2 text-[14px] font-semibold mb-[22px] text-brand-700"> <svg className="w-[16px] h-[16px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5M12 19l-7-7 7-7" /></svg> {t("blogArticle.allArticles")} </LocalizedLink>
            <div className="flex items-center justify-center gap-3 mb-[18px]">
              <Chip>{t("blogArticle.tag")}</Chip>
              <span className="text-muted text-[13px]">{t("blogArticle.readTime")}</span>
            </div>
            <h1 className="display" style={{ fontSize: "clamp(30px,4vw,50px)" }}>{t("blogArticle.title")}</h1>
            <div className="flex items-center justify-center gap-3 mt-[26px]">
              ...
              <div className="text-left">
                <div className="font-bold text-[14px]">{t("blogArticle.author")}</div>
                <div className="text-muted text-[12.5px]">{t("blogArticle.authorMeta")}</div>
              </div>
            </div>
          </header>
          <div className="container mt-[40px] max-w-[960px]">
            <Placeholder label={t("blogArticle.heroImage")} className="rounded-lg" style={{ height: "clamp(240px,34vw,440px)" }} />
          </div>
          {/* BODY */}
          <div className="container prose max-w-[720px]">
            <p className="lede">{t("blogArticle.lede")}</p>
            <p>{t("blogArticle.intro")}</p>
            ...
            <h2>{sections[1].h}</h2>
            <p>{sections[1].p}</p>
            <blockquote>{t("blogArticle.blockquote")}</blockquote>

            <h2>{sections[2].h}</h2>
            <p>{sections[2].p}</p>
            <ul>
              {checkoutList.map((li) => <li key={li}>{li}</li>)}
            </ul>
            ...
            <p>{t("blogArticle.conclusion")}</p>
            <div className="callout">
              <div className="font-bold text-[16px] mb-[6px]">{t("blogArticle.callout.title")}</div>
              <p className="mt-0 mb-[16px] text-muted text-[15px] leading-[1.65]">{t("blogArticle.callout.body")}</p>
              <Button asChild><LocalizedLink href={ROUTES.CONTACT}>{t("blogArticle.callout.cta")}</LocalizedLink></Button>
            </div>
          </div>
          {/* SHARE */}
          <div className="container mt-[8px] max-w-[720px]">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-muted text-[13.5px]">{t("blogArticle.share")}</span>
                <SocialLink label={t("blogArticle.shareXAria")} href={EXTERNAL.SHARE_X} path={SOCIAL_PATHS.x} style={SHARE_ICON_STYLE} />
                <SocialLink label={t("blogArticle.shareLinkedInAria")} href={EXTERNAL.SHARE_LINKEDIN} path={SOCIAL_PATHS.linkedin} style={SHARE_ICON_STYLE} />
              </div>
            </div>
          </div>
        </article>
        ...
        <section className="section bg-cream" style={{ marginTop: "clamp(48px,6vw,80px)" }}>
          <div className="container">
            <h2 className="display mb-[32px]" style={{ fontSize: "clamp(22px,2.6vw,32px)" }}>{t("blogArticle.keepReading")}</h2>
